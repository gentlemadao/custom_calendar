...
import 'package:custom_calendar/widget/month_view.dart';
...
class CalendarProvider extends ChangeNotifier {
  ...
  HashSet<DateModel>? selectedDateList = HashSet<DateModel>(); //被选中的日期,用于多选
  ...
  ValueNotifier<int> _generation = ValueNotifier(0); //生成的int值，每次变化，都会刷新整个日历。
...
//    notifyListeners();
  ...
  int get weekPageIndex {
    ...
    for (int i = 0; i < calendarConfiguration.weekList.length; i++) {
      DateTime nextWeek = firstWeek.add(const Duration(days: 7));
      //减去偏移量
      if (dateModel != null &&
          dateModel.getDateTime().isBefore(nextWeek
              .subtract(Duration(days: calendarConfiguration.offset)))) {
        index = i;
        break;
      } else {
        firstWeek = nextWeek;
  ...
  int get monthPageIndex {
    ...
    for (int i = 0; i < calendarConfiguration.monthList.length - 1; i++) {
      ...
      if (dateModel != null &&
          !dateModel.getDateTime().isBefore(preMonth) &&
  ...
  void initData({
    ...
    if (calendarConfiguration.defaultSelectedDateList != null) {
      this
          .selectedDateList
          ?.addAll(calendarConfiguration.defaultSelectedDateList!);
    }
